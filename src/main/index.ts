...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('ahacode', process.execPath, [
      path.resolve(process.argv[1]),
    ])
  }
}
else {
  app.setAsDefaultProtocolClient('ahacode')
}
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    protocol.handle('ahacode', async (request) => {
  ...
  app.on('second-instance', (_, argv) => {
    ...
    if (process.platform !== 'darwin') {
      const url = argv.find(i => i.startsWith('ahacode://'))
