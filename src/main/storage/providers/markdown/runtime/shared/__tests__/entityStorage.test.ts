...
describe('deleteEntityFromStateAndDisk', () => {
  ...
  it('removes file and entries from state and runtime', () => {
    const rootPath = createTempRoot()
    fs.ensureDirSync(path.join(rootPath, '.ahacode', 'inbox'))
    fs.writeFileSync(
      path.join(rootPath, '.ahacode', 'inbox', 'one.md'),
      '# one',
    )
    ...
    expect(
      fs.pathExistsSync(path.join(rootPath, '.ahacode', 'inbox', 'one.md')),
    ).toBe(false)
...
describe('emptyEntityTrashFromStateAndDisk', () => {
  ...
  it('removes deleted files and keeps non-deleted entries', () => {
    const rootPath = createTempRoot()
    fs.ensureDirSync(path.join(rootPath, '.ahacode', 'inbox'))
    fs.ensureDirSync(path.join(rootPath, '.ahacode', 'trash'))
    fs.writeFileSync(
      path.join(rootPath, '.ahacode', 'inbox', 'keep.md'),
      '# keep',
    )
    fs.writeFileSync(
      path.join(rootPath, '.ahacode', 'trash', 'drop.md'),
      '# drop',
    )
    ...
    expect(
      fs.pathExistsSync(path.join(rootPath, '.ahacode', 'trash', 'drop.md')),
    ).toBe(false)
    expect(
      fs.pathExistsSync(path.join(rootPath, '.ahacode', 'inbox', 'keep.md')),
    ).toBe(true)
