...
describe('getPaths', () => {
  it('detects existing data in legacy root vault layout', () => {
    ...
    fs.ensureDirSync(path.join(vaultPath, '.ahacode'))
    fs.writeJSONSync(path.join(vaultPath, '.ahacode', 'state.json'), {
      counters: {
        contentId: 0,
        folderId: 1,
        snippetId: 1,
    ...
    expect(
      fs.pathExistsSync(
        path.join(vaultPath, 'code', '.ahacode', 'state.json'),
      ),
    ).toBe(true)
  })
  ...
  it('migrates legacy code vault root into code', () => {
    ...
    expect(
      fs.pathExistsSync(path.join(paths.vaultPath, '.ahacode', 'state.json')),
    ).toBe(true)
    expect(
      fs.pathExistsSync(path.join(paths.vaultPath, 'Work', 'demo.md')),
    ).toBe(true)

    expect(fs.pathExistsSync(path.join(vaultPath, '.ahacode'))).toBe(false)
  ...
  it('keeps existing initialized code space without forced root migration', () => {
    ...
    fs.ensureDirSync(path.join(codeRoot, '.ahacode'))
    fs.writeJSONSync(path.join(codeRoot, '.ahacode', 'state.json'), {
      counters: {
        contentId: 0,
        folderId: 0,
        snippetId: 0,
  ...
  it('flattens __spaces__ wrapper for all known spaces before resolving code path', () => {
    ...
    fs.ensureDirSync(path.join(legacyCodeRoot, '.ahacode'))
    fs.writeJSONSync(path.join(legacyCodeRoot, '.ahacode', 'state.json'), {
      counters: {
        contentId: 0,
        folderId: 0,
        snippetId: 0,
