...
describe('getNotesPaths', () => {
  it('migrates nested notes space from legacy code root to flat notes root', () => {
    ...
    fs.ensureDirSync(path.join(legacyNotesRoot, '.ahacode'))
    fs.writeFileSync(
      path.join(legacyNotesRoot, '.ahacode', 'state.json'),
      '{"version":2}',
      'utf8',
    )
  ...
  it('merges nested notes space into existing flat notes space', () => {
    ...
    fs.ensureDirSync(path.join(notesRoot, '.ahacode'))
    fs.writeFileSync(
      path.join(notesRoot, '.ahacode', 'state.json'),
      '{"version":2}',
      'utf8',
    )
